...
class Database:
    ...
    async def init(self):
        ...
        async with self.pool.acquire() as conn:
            ...
            # ----------------------------------------------------------------
            # FIX: Removed "DROP TABLE IF EXISTS chats" that was wiping the
            # chat list on every restart. Now uses CREATE TABLE IF NOT EXISTS
            # so existing registered chats survive redeployments.
            # Also adds the column as BIGINT in case the table is brand new.
            # ----------------------------------------------------------------
            await conn.execute('''
            ...
            # Index for fast pending-trade lookups
            await conn.execute(
    ...
    async def mark_partial_notified(self, trade_id: int):
        async with self.pool.acquire() as conn:
            await conn.execute(
                "UPDATE trades SET partial_notified = 1 WHERE id = $1",
                trade_id,
            )
    ...
    async def get_recent_trades(self, limit: int = 10) -> List[Dict]:
        async with self.pool.acquire() as conn:
            rows = await conn.fetch(
                """
                SELECT pair, direction, entry, sl, tp, score, status, timestamp
                FROM trades
    ...
    # ------------------------------------------------------------------
    # Anti-spam signal timing
    # ------------------------------------------------------------------

    async def get_last_signal_time(self, pair: str) -> float:
        async with self.pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT last_time FROM last_signal WHERE pair = $1", pair
            )
    ...
    async def set_last_signal_time(self, pair: str, timestamp: float):
        async with self.pool.acquire() as conn:
            await conn.execute(
                """
                INSERT INTO last_signal (pair, last_time) VALUES ($1, $2)
                ON CONFLICT (pair) DO UPDATE SET last_time = EXCLUDED.last_time
                """,
                pair, timestamp,
            )
    ...
    async def get_pair_performance(self) -> Dict[str, Tuple[int, int]]:
        async with self.pool.acquire() as conn:
            ...
            result = {}
            for row in rows:
                pair, status, count = row['pair'], row['status'], row['count']
                ...
                if status == 'WIN':
                    ...
                    result[pair][1] = count
            return {pair: (w, l) for pair, (w, l) in result.items()}
